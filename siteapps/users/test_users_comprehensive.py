...
class UsersAPITestCase(TestCase):
    ...
    def test_superuser_edit_other_user_staff(self):
        """Test that superuser can edit another user's staff status"""
        # Ensure the authenticated user is a superuser
        self.user.is_superuser = True
        self.user.is_staff = True
        self.user.save()

        # Create a second user who is not staff
        second_user = User.objects.create(email='seconduser@example.com')
        second_user.set_password("SecurePassword123")
        second_user.save()
        self.assertEqual(second_user.is_staff, False)

        # Grant staff role to second user
        response = self.client.post(
            "/v1/users/edit_staff", {"accountEmail": second_user.email, "setStaff": True}, format="json"
        )
        second_user.refresh_from_db()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(second_user.is_staff, True)

        # Revoke staff role from second user
        response = self.client.post(
            "/v1/users/edit_staff", {"accountEmail": second_user.email, "setStaff": False}, format="json"
        )
        second_user.refresh_from_db()
        self.assertEqual(response.status_code, 200)
        self.assertEqual(second_user.is_staff, False)
